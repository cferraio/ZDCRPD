...
static const int ICH_LUM_MAX = 16; //actually the RPD in disguise 
...
bool ZdcTopology::isExcluded(const HcalZDCDetId& id) const {
  ...
  case(HcalZDCDetId::RPD) : exed = excludeLUM_; break;
  default: exed = false;
  }
...
void ZdcTopology::exclude(int zside, HcalZDCDetId::Section section) {
  ...
  case(HcalZDCDetId::RPD) : excludeLUM_ = true; break;
  default: break;
  }
}
...
bool ZdcTopology::validRaw(const HcalZDCDetId& id) const{
  ...
  if(id.section()== HcalZDCDetId::HAD && id.channel() > ICH_HAD_MAX)
    return false;
  if(id.section()== HcalZDCDetId::RPD && id.channel() > ICH_LUM_MAX)
    return false;
  return ok;
}
...
int ZdcTopology::ncells(HcalZDCDetId::Section section) const{
    ...
    switch (section) {
        ...
        case(HcalZDCDetId::RPD) : ncells = ICH_LUM_MAX; break;
...
int ZdcTopology::firstCell(HcalZDCDetId::Section section)const {
    ...
    switch (section) {
        ...
        case(HcalZDCDetId::RPD) : firstCell = firstLUMModule_; break;
...
int ZdcTopology::lastCell(HcalZDCDetId::Section section) const {
    ...
    switch (section) {
        ...
        case(HcalZDCDetId::RPD) : lastCell = lastLUMModule_; break;
